...
(Exported). Prefer the engine input helper when the engine is known:

```
scripts/spark_input.py --table catalog.db.tbl --out-dir input_bundle
scripts/trino_input.py --table catalog.db.tbl --out-dir input_bundle
scripts/glue_input.py --table db.tbl --out-dir input_bundle
scripts/snowflake_input.py --table db.schema.tbl --out-dir input_bundle
```

These helpers create exact read-only export SQL plus `run_profile.sh` and
`run_workload.sh`. For Flink/Kafka Connect writers, collect the Iceberg catalog
profile through Spark, Glue/EMR, Trino, or Snowflake, then use
`engines/ingestion.md` for writer configuration.

When metadata CSVs are available, feed them to the profiler:
...
Snowflake query-history CSVs generated by `snowflake_input.py`, or Trino
`EXPLAIN ANALYZE` (run `--help` for the fields each reads). If none are
available, ask the user to run the relevant engine input helper or `EXPLAIN
ANALYZE` on 3–5 representative queries.
...
`parse_query_log.py`, and `simulate.py` under `scripts/`.
