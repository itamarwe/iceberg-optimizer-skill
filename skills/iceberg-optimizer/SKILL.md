...
1. **Gateway / Direct** — the user already has an Iceberg-capable gateway or CLI
   (`trino`, `spark-sql`, `beeline`, Snowflake, Glue/EMR, REST catalog endpoint,
   or env `TRINO_URL` / `SPARK_HOME` / `DATABRICKS_HOST` / `SNOWFLAKE_ACCOUNT`).
...
2. **Exported** — the user provided files (`profile.json`, metadata CSVs, query
   history); read those files and do not ask for live access.
...
decision-framework, scheduling) and `engines/` (spark, glue, trino, duckdb, snowflake,
